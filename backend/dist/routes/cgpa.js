...
const calculateCGPA = async (userId, currentSemesterGPA, currentCredits) => {
    const previousRecords = await CGPARecord_1.CGPARecord.find({ userId });
...
router.post('/record', [
    ...
    try {
        ...
        const existingRecord = await CGPARecord_1.CGPARecord.findOne({ userId, semester, year });
        if (existingRecord) {
            ...
            // Create new record
            const cgpaRecord = new CGPARecord_1.CGPARecord({
                userId,
                semester,
                year,
...
const recalculateAllCGPA = async (userId) => {
    const records = await CGPARecord_1.CGPARecord.find({ userId }).sort({ year: 1, semester: 1 });
...
router.get('/record/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.userId;
        const recordId = req.params.id;
        const record = await CGPARecord_1.CGPARecord.findOne({ _id: recordId, userId });
        if (!record) {
...
// Delete CGPA record
router.delete('/record/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.userId;
        const recordId = req.params.id;
        const record = await CGPARecord_1.CGPARecord.findOneAndDelete({ _id: recordId, userId });
        if (!record) {
            return res.status(404).json({
                success: false,
                message: 'CGPA record not found'
...
router.get('/grade-points', (req, res) => {
    res.json({
        success: true,
        data: {
            gradePointMap,
